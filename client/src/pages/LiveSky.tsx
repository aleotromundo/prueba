...
import { AlertCircle, Camera, Check, Compass, Crosshair, Eye, EyeOff, LocateFixed, Moon, RefreshCw, ShieldCheck, Sparkles, Sun, Target, ZoomIn } from "lucide-react";
import { Body, Equator, Horizon, Observer } from "astronomy-engine";
import { ToolLayout } from "@/components/ToolLayout";
import WebXRARSession from "@/components/WebXRARSession";
import { detectWebXRAR, type WebXRState } from "@/lib/webxrSupport";
...
export default function LiveSky() {
  ...
  const [planetariumEngine, setPlanetariumEngine] = useState<"stellarium" | "local">("stellarium");
  const [calibrationOffset, setCalibrationOffset] = useState(0);
  const [jplState, setJplState] = useState<JplState>("idle");
  const [jplMessage, setJplMessage] = useState("");
  const [webxrState, setWebxrState] = useState<WebXRState>("checking");

  useEffect(() => {
    const xr = (navigator as Navigator & { xr?: { isSessionSupported?: (mode: string) => Promise<boolean> } }).xr;
    void detectWebXRAR(window.location, xr).then(setWebxrState);
  ...
  const toggleCamera = () => {
    if (webxrState !== "supported") {
      setCameraEnabled(false);
      setJplMessage(webxrState === "insecure" ? "WebXR AR requiere una conexión HTTPS segura." : "Tu navegador o dispositivo no es compatible con WebXR AR. El planetario local sigue disponible, pero no voy a mostrar una falsa superposición de cámara.");
      return;
    }
    if (cameraEnabled) {
      setCameraEnabled(false); return;
    }
  ...
  return <ToolLayout eyebrow="Cielo en vivo" title="Apuntá tu teléfono al cielo" intro="Un planetario local que combina tu hora, tu ubicación y la orientación del dispositivo con cálculos astronómicos ejecutados en el navegador.">
    <section className="live-sky-intro"><div><span className="eyebrow"><Sparkles size={14} /> Vista de planetario y AR</span><h2>El cielo, ahora.</h2><p>Elegí planetario local o WebXR AR. La sesión AR solo aparece si tu dispositivo confirma compatibilidad real; si no, lo informamos sin mostrar una simulación.</p></div><div className="live-sky-status"><span><ShieldCheck size={15} /> Tus datos no se guardan</span><span><RefreshCw size={15} /> Actualización en tiempo real</span></div></section>
    ...
    <section className="live-sky-tools" aria-label="Controles del visor"><button className={`button ${cameraEnabled ? "button--active" : "button--outline"}`} onClick={toggleCamera} disabled={webxrState === "checking"}><Camera size={15} /> {webxrState === "supported" ? (cameraEnabled ? "Salir de WebXR AR" : "Iniciar WebXR AR") : webxrState === "checking" ? "Comprobando WebXR…" : "WebXR no compatible"}</button><span className={`live-sky-xr-status live-sky-xr-status--${webxrState}`}><AlertCircle size={14} /> {webxrState === "supported" ? "AR compatible" : webxrState === "insecure" ? "Requiere HTTPS" : webxrState === "checking" ? "Comprobando dispositivo" : "Dispositivo no compatible"}</span><button className={`button ${nightMode ? "button--active" : "button--outline"}`} onClick={() => setNightMode((value) => !value)}>{nightMode ? <Sun size={15} /> : <Moon size={15} />} {nightMode ? "Modo claro" : "Modo nocturno"}</button><button className={`button ${planetariumEngine === "stellarium" ? "button--active" : "button--outline"}`} onClick={() => setPlanetariumEngine((value) => value === "stellarium" ? "local" : "stellarium")}><Sparkles size={15} /> {planetariumEngine === "stellarium" ? "Stellarium activo" : "Usar Stellarium"}</button><button className="button button--outline" onClick={() => setShowLabels((value) => !value)}>{showLabels ? <EyeOff size={15} /> : <Eye size={15} />} {showLabels ? "Ocultar etiquetas" : "Mostrar etiquetas"}</button><label className="live-sky-range"><ZoomIn size={15} /> FOV <input type="range" min="60" max="180" value={fieldOfView} onChange={(event) => setFieldOfView(Number(event.target.value))} /><strong>{fieldOfView}°</strong></label></section>
    <section className={`live-sky-horizon-view ${nightMode ? "live-sky-horizon-view--night" : ""}`} aria-label={planetariumEngine === "stellarium" ? "Vista Stellarium del cielo profundo" : "Vista de planetario local"}>{planetariumEngine === "stellarium" ? <StellariumPlanetarium latitude={coordinates.latitude} longitude={coordinates.longitude} date={now} heading={orientation.heading} onSelect={(object) => { if (object) selectObject(object.name); }} /> : <div className={`live-sky-scene ${cameraEnabled ? "live-sky-scene--camera" : ""}`}><div className="live-sky-camera-layer" aria-hidden={!cameraEnabled}>{cameraEnabled && <WebXRARSession objects={visibleObjects} initialHeading={orientation.heading} onStateChange={(state, message) => { if (state === "error" || state === "unsupported") setJplMessage(message ?? "WebXR AR no disponible."); }} />}</div><div className="live-sky-scene__veil" /><div className="live-sky-scene__atmosphere" /><div className="live-sky-scene__topbar"><span>N {Math.round(orientation.heading)}°</span><span>{direction}</span><span>Campo de visión {fieldOfView}°</span></div><div className="live-sky-scene__horizon"><span>Horizonte · 0°</span><i /><span>Zénit · 90°</span></div><svg viewBox="0 0 100 100" role="img" aria-label="Objetos celestes calculados en tiempo real"><defs><filter id="sceneGlow"><feGaussianBlur stdDeviation=".8" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter></defs>{projectedObjects.map(({ object, point }) => { const isBody = object.kind === "body"; const isFeatured = isBody || (object.magnitude ?? 9) <= .55; const size = isBody ? (object.name === "Sol" ? 1.45 : object.name === "Luna" ? 1.25 : .95) : Math.max(.45, 1.15 - (object.magnitude ?? 1) * .18); return <g key={object.name} className={`live-sky-object ${isBody ? "live-sky-object--body" : "live-sky-object--star"} ${selected === object.name ? "live-sky-object--selected" : ""}`} onClick={() => selectObject(object.name)} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); selectObject(object.name); } }} aria-label={`${object.name}, azimut ${formatAngle(object.azimuth)}, altura ${object.altitude.toFixed(1)} grados`}>{isBody && <circle cx={point.x} cy={point.y} r={size * 2.1} fill={object.color} opacity=".3" filter="url(#sceneGlow)" />}{!isBody && <circle cx={point.x} cy={point.y} r={size * 1.5} fill={object.color} opacity=".5" filter="url(#sceneGlow)" />}<circle cx={point.x} cy={point.y} r={size} fill={object.color} stroke={selected === object.name ? "#fff" : "none"} strokeWidth=".55" filter="url(#sceneGlow)" />{showLabels && isBody ? <text className="live-sky-scene-label live-sky-scene-label--body" x={point.x + 2.5} y={point.y - 1.5} fill={object.color}>{selected === object.name ? `${object.symbol} ${object.name}` : object.symbol}</text> : showLabels && isFeatured && <text className="live-sky-scene-label" x={point.x + 2.5} y={point.y - 1.5} fill="#eef4ff">{object.symbol} {object.name}</text>}</g>; })}</svg><div className="live-sky-crosshair" aria-hidden="true"><span /><span /></div><div className="live-sky-scene__footer"><span>{cameraEnabled ? "WebXR AR activo · tracking espacial del dispositivo" : "Planetario local · escena calculada"}</span><span>Orientación por sensores o rumbo manual</span></div></div>}<p className="live-sky-attribution">{cameraEnabled ? "WebXR AR: la cámara y el tracking pertenecen a la sesión XR; la alineación inicial requiere apuntar al norte." : "Fondo panorámico: ESO/S. Brunier · CC BY 4.0"} · <a href="https://www.eso.org/public/images/eso0932a/" target="_blank" rel="noreferrer">ver fuente</a></p></section>
    ...
    <aside className="live-sky-note"><Crosshair size={18} /><p><strong>Cómo leerlo.</strong> El planetario local calcula las posiciones en tu dispositivo. WebXR AR solo se habilita cuando el navegador confirma soporte `immersive-ar`; si tu equipo no es compatible, lo indicamos claramente y no mostramos una cámara disfrazada de realidad aumentada. El modo manual sigue funcionando.</p></aside>
  </ToolLayout>;
}
